...
module.exports ={
  ...
  createGameTypeTeam: function(user_id, teammate_id, game_id){
      return knex.raw(`insert into teams_games (user_id, teammate_id, game_id) values (${user_id}, ${teammate_id}, ${game_id})`);
      //Julie's code
  },
  ...
  readGameStandings: function(game_ids){
    return knex.raw(`SELECT e.rating, e.user_id, e.user_game_id, a."userName", l.user_id as user_id__games, l.game_id as game_id__games
      FROM user_game_stats e
      JOIN users a
      ON a.id=e.user_id
      JOIN user_games l
      ON game_id=l.game_id
      WHERE game_id=2
